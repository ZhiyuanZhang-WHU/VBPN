...
def padding(x, bench_size=8, mode='replicated', value=0):
    h, w = x.size()[-2:]
    ...
    if mode.upper() == 'constant'.upper():
        x = torch.nn.ConstantPad2d(padding=(padding_left, padding_right, padding_top, padding_bottom), value=value)(x)
    elif mode.upper() == 'replicated'.upper():
        x = torch.nn.ReplicationPad2d(padding=(padding_left, padding_right, padding_top, padding_bottom))(x)
